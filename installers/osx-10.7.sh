...
#
# osx-10.7.sh
#           installation script for phix on osx-10.7
#
...
pecl_module() {
    ...
    if [[ $? == 1 ]] ; then
        ...
        echo "extension=$1.so" >> /etc/php.ini
    else
...
if [[ $? != 0 ]] ; then
    die "Cannot find the PHP CLI in your PATH; please investigate why"
fi
...
# step 2: we need to upgrade PEAR
#
# we cannot be certain that the user has the latest PEAR installer
